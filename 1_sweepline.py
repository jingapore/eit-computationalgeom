...
class Event:
    ...
    def __init__(self, type, segment, segment2=None, ipoint=None):
        ...
        if type == 1:
            if segment.y1 < segment.y2 or (
                segment.y1 == segment.y2 and segment.x2 < segment.x1
            ):
...
def checkIntersection(pos, pos2, Events, Status):
    ...
    if (
        ipoint and ipoint[1] < segment.currentYList[0]
    ):  # <segment.currentYList[0] checks if sweepline is above, i.e. has NOT passed, intersection point
...
def handleStartEvent(segment, Events, Status):
    ...
    Status.add(segment)
    newSegment_index = Status.index(segment)
    left_of_newSegment = newSegment_index - 1
    right_of_newSegment = newSegment_index + 1
    if left_of_newSegment >= 0:
        checkIntersection(left_of_newSegment, newSegment_index, Events, Status)
    if right_of_newSegment + 1 < len(Status):  # +1 because Python indexes from 0
        checkIntersection(right_of_newSegment, newSegment_index, Events, Status)
...
def handleEndEvent(segment, Events, Status):
    ...
    # TODO: Implement EventHandling for the upper endpoint of a segment
    # If you don't know where to start, you can look at the event handling for intersection events
    left_of_removedSegment = Status.index(segment)-1
    right_of_removedSegment = Status.index(segment)+1
    if left_of_removedSegment >= 0 and right_of_removedSegment + 1 < len(Status):
        checkIntersection(left_of_removedSegment, right_of_removedSegment, Events, Status)
    Status.remove(segment)


def handleIntersectionEvent(segment, segment2, Events, Status):
    currentY = segment.currentYList[0]
    segment.currentYList[0] = (
        currentY + 0.00001
...
def sweep(inputSegments):
    ...
    while Events:
        e = (
            Events.pop()
        )  # removes from end of SortedList, i.e. start with largest y value
        nEvents += 1
...
input = readSegments("1_linesegments.txt")
sweep(input)
